...
namespace Animal.Controllers.ApiController
{
    ...
    [ApiController]
    public class FeedFooderController : ControllerBase
    {
        // private readonly IRepository<FeedFooder> _repo;
        private readonly IUnitOfWork _repoU;
        private readonly IMapper _mapper;

        public FeedFooderController(IUnitOfWork repoU, IMapper mapper)
        {
            //   _repo = repo;
            _repoU = repoU;
            _mapper = mapper;
        ...
        [HttpPost("addeditfeed/{id}")]
        public ActionResult AddEditFeed(int? id, [FromForm]FeedFooder model)
        {
            try
            {
        ...
        public IActionResult FeedFooderCategory(string category)
        {
            if (category == "Green" || category == "Dry" || category == "Mineral")
            {
                var feed = _mapper.Map<FeedFooderDto>(_repoU.FeedFooder.GetByCategory(category));

                return Ok(feed);
            }
            return BadRequest("Can't find the category name. Please Use valid category name.");
        }
        ...
        //}
        [HttpPost("[action]")]
        public IActionResult milkBaseCalculation([FromForm]List<FeedCalculator> feeds)
        {
            List<FeedCalculator> feedResult = new List<FeedCalculator>();
            string fat = feeds[0].fatPercentage.ToString();
            string type = feeds[0].milk;
