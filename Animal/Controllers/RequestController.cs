...
namespace Animal.Controllers
{
    ...
    public class RequestController : Controller
    {
        ...
        [HttpPost]
        public IActionResult CheckEarTag(string earTag)
        {
        ...
        [HttpPost]
        public IActionResult GetBull(string id)
        {
            string earTag = _repo.AnimalRegistration.GetById(int.Parse(id)).earTagNo;
            Ai Ais = _repo.Ais.GetModel().FirstOrDefault(m => m.earTagNo == earTag);
            IEnumerable<AnimalRegistration> animals = _repo.AnimalRegistration.GetModel().Where(m => m.gender.ToUpper() == "MALE");
            if (Ais != null)
            {
                string BullTag = Ais.BullEarTag;
                animals = _repo.AnimalRegistration.GetModel().Where(m => m.gender.ToUpper() == "MALE" && m.earTagNo != BullTag);

            }
            return Json(animals);
        }
        [HttpPost]
        public IActionResult CheckEarTagUsed(string earTag)
        {
        ...
        public IActionResult CascadeBreed(string speciesId)
        {
        ...
        [HttpPost]
        public IActionResult AllowAccess(string userId)
        {
            try
            {
